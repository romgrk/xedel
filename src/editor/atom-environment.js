...
const CommandRegistry = require('../commands-manager');
...
class Environment {
  ...
  */
  ...
  constructor(params = {}) {
    ...
    // FIXME: fix this, requires keymap-extensions from atom
  ...
  initialize(params = {}) {
    ...
    // this.themes.loadBaseStylesheets(); // FIXME
  ...
  */
  ...
  async displayWindow() {
    ...
    const steps = [this.show(), this.focus()];
  ...
  getDefaultWindowDimensions() {
    return { x: 100, y: 100, width: 800, height: 800 };
  ...
  async startEditorWindow() {
    ...
    const loadStatePromise = this.loadState().then(async state => {
      ...
      this.window.setChild(this.workspace.getElement());
