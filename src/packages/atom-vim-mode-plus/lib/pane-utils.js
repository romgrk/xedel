...
function maximizePane (centerPane) {
  ...
  atom.workspace.getElement().addCssClass(...workspaceClassList)

  const activePaneElement = atom.workspace.getActivePane().getElement()
  activePaneElement.addCssClass('vim-mode-plus--active-pane')
  for (const element of atom.workspace.getElement().getElementsByTagName('atom-pane-axis')) {
    if (element.contains(activePaneElement)) {
      element.addCssClass('vim-mode-plus--active-pane-axis')
    }
  }
}

function demaximizePane () {
  if (isMaximized()) {
    const workspaceElement = atom.workspace.getElement()
    workspaceElement.removeCssClass(
      'vim-mode-plus--pane-maximized',
    ...
    for (const element of workspaceElement.getElementsByTagName('atom-pane-axis')) {
      element.removeCssClass('vim-mode-plus--active-pane-axis')
    }
    for (const element of workspaceElement.getElementsByTagName('atom-pane')) {
      element.removeCssClass('vim-mode-plus--active-pane')
    }
  }
}
