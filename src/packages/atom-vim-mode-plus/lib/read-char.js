...
function swapClassName (vimState, ...classNames) {
  ...
  vimState.editorElement.removeCssClass('vim-mode-plus', oldMode + '-mode')
  vimState.editorElement.addCssClass(...classNames)

  return new Disposable(() => {
    vimState.editorElement.removeCssClass(...classNames)
    const classToAdd = ['vim-mode-plus', 'is-focused']
    if (vimState.mode === oldMode) classToAdd.push(oldMode + '-mode')
    vimState.editorElement.addCssClass(...classToAdd)
  })
}
