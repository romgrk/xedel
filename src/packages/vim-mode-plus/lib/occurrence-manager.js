...
const {shrinkRangeEndToBeforeNewLine, collectRangeByScan} = require('./utils')
...
module.exports = class OccurrenceManager {
  ...
  markBufferRangeByPattern (regex, occurrenceType) {
    const {editor} = this.vimState

    let occurrenceRanges = collectRangeByScan(editor, regex)

    if (occurrenceType === 'subword') {
      const subwordRegex = editor.getLastCursor().subwordRegExp()
      const subwordRangesByRow = {}
      occurrenceRanges = occurrenceRanges.filter(range => {
        const row = range.start.row
        if (!subwordRangesByRow[row]) {
          subwordRangesByRow[row] = collectRangeByScan(editor, subwordRegex, {row})
        }
        return subwordRangesByRow[row].some(subwordRange => subwordRange.isEqual(range))
      })
    }

    if (this.canCreateMarkersForLength(occurrenceRanges.length)) {
      occurrenceRanges.forEach(range => {
  ...
  // Return occurrence markers intersecting with given selection
  getMarkersIntersectsWithSelection (selection, exclusive = false) {
    ...
    // But basically I should prefer findMarker since It's faster than iterating
    // every markers manually.
