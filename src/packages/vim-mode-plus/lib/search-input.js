...
module.exports = class SearchInput {
  ...
  constructor (vimState) {
    ...
    this.editorElement.addCssClass('vim-mode-plus-search')
  ...
  focus (options = {}) {
    ...
    if (this.options.classList != null) {
      this.editorElement.addCssClass(...this.options.classList)
    }
    this.panel.show()
    ...
    this.vimState.editorElement.addCssClass('vim-mode-plus-search-input-focused')
  ...
  hasFocus () {
    return this.focusSubscriptions != null
  }

  unfocus () {
    this.finished = true
    this.vimState.editorElement.removeCssClass('vim-mode-plus-search-input-focused')

    if (this.options.classList) {
      this.editorElement.removeCssClass(...this.options.classList)
    }
    this.regexSearchStatus.addCssClass('btn-primary')
    if (this.literalModeDeactivator) this.literalModeDeactivator.dispose()

    if (this.focusSubscriptions) {
      this.focusSubscriptions.dispose()
    }
    this.focusSubscriptions = null
  ...
  activateLiteralMode () {
    if (this.literalModeDeactivator) {
      ...
      this.editorElement.addCssClass('literal-mode')

      this.literalModeDeactivator.add(
        new Disposable(() => {
          this.editorElement.removeCssClass('literal-mode')
          this.literalModeDeactivator = null
        })
      )
    }
